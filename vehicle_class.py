...
class vehicle:
	...
	def check_goal_9x9(vehicle, vehicle_dict, archive, df_grid, child_df_grid, find_path, grid, start_time):
		...
		if vehicle == mycar:
			...
			if vehicle_dict[vehicle][1] == goal:
				...
				total_moves = 0
				# create solution path by reconstructing child to parent boards
	...
	# method to check if goal is reached for 6x6 boards
	@staticmethod
	...
	def move(vehicle, vehicle_dict, df_grid):
		...
		# check if its possible to move to new coordinate, then change coordinates accordingly
